...
from uelc.main.tests.factories import GroupUpFactory, FacilitatorUpFactory, \
    AdminUpFactory
...
class TestFacilitatorLoggedInViews(TestCase):
    def setUp(self):
        ...
        self.usr = FacilitatorUpFactory().user
        self.client = Client()
        self.client.login(username=self.usr.username, password="test")
...
class TestAdminLoggedInViews(TestCase):
    def setUp(self):
        ModuleFactory("main", "/pages/main/")
        self.hierarchy = Hierarchy.objects.get(name='main')
        self.section = self.hierarchy.get_root().get_first_leaf()

        self.usr = AdminUpFactory().user
        self.client = Client()
        self.client.login(username=self.usr.username, password="test")
